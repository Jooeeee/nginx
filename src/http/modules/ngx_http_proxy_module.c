...
static ngx_command_t ngx_http_proxy_commands[] = {
    ...
    {ngx_string("proxy_detect_buffer_size"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_size_slot,
     NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_proxy_loc_conf_t, upstream.detect_buffer_size),
     NULL},
...
extern ngx_int_t test_get_and_print_request(ngx_http_request_t *r);
static ngx_int_t
...
ngx_http_proxy_create_loc_conf(ngx_conf_t *cf)
{
    ...
    conf->upstream.detect_buffer_size = NGX_CONF_UNSET_SIZE;
...
ngx_http_proxy_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_size_value(conf->upstream.detect_buffer_size,
                              prev->upstream.detect_buffer_size, 0);
...
ngx_http_proxy_set_ssl(ngx_conf_t *cf, ngx_http_proxy_loc_conf_t *plcf)
{
    ...
    if (ngx_ssl_client_session_cache(cf, plcf->upstream.ssl,
                                     plcf->upstream.ssl_session_reuse) != NGX_OK)
    {
        return NGX_ERROR;
    }
