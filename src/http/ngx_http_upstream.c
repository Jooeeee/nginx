...
static ngx_int_t ngx_http_upstream_send_request_body(ngx_http_request_t *r,
                                                     ngx_http_upstream_t *u, ngx_uint_t do_write);
static ngx_int_t ngx_http_upstream_send_request_body_after_check(ngx_http_request_t *r,
                                                                 ngx_http_upstream_t *u, ngx_uint_t do_write);
                                          ...
                                          ngx_http_upstream_t *u)
{
    ...
    if (ngx_http_subrequest(r, &r->uri, &r->args, &sr, NULL,
                            NGX_HTTP_SUBREQUEST_CLONE | NGX_HTTP_SUBREQUEST_BACKGROUND) != NGX_OK)
    {
        return NGX_ERROR;
    }
                               ...
                               ngx_uint_t do_write)
{
    ...
    rc = ngx_http_upstream_send_request_body_after_check(r, u, do_write);
...
static ngx_int_t
ngx_http_upstream_send_request_body_after_check(ngx_http_request_t *r,
                                                ngx_http_upstream_t *u, ngx_uint_t do_write)
{
    ngx_int_t rc;
    ...
    if (!r->request_body_no_buffering)
    {
        ...
        rc = ngx_http_request_body_detector(r, out);
        if (rc != NGX_OK)
        {
            return rc;
        }
...
//接收和解析上游服务器的响应。再完成提取响应头并把响应头放到heads_in.headers中后，进入发送流程。
static void
ngx_http_upstream_process_header(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    /* rc == NGX_OK */
    // 处理已经解析出的头部，该函数会把已经解析出的头部
    u->state->header_time = ngx_current_msec - u->start_time;
    // 判断头部是否特殊响应，则进行next的进一步决策处理，没有必要继续处理响应
...
ngx_http_upstream_test_next(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    for (un = ngx_http_upstream_next_errors; un->status; un++)
    {
        ...
        if (u->peer.tries > 1 && ((u->conf->next_upstream & mask) == mask) && !(u->request_sent && r->request_body_no_buffering) && !(timeout && ngx_current_msec - u->peer.start_time >= timeout))
        {
...
//将响应发送给客户端。通过buffering可以配置选择哪种模式：
//1. 上游网速优先，buffering=1。上游网速较快，响应来不及转发，因此尽可能使用内存/文件缓存上游发送来的响应。
//2. 下游网速优先，buffering=0。下游网速较快，响应能及时转发，因此可采用固定大小的内存作为响应的缓冲区。
void ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ssize_t n;
    ngx_int_t rc;
    ngx_event_pipe_t *p;
    ...
    if (!u->buffering)
    {
        ...
        // 检测缓存中是否还有报文。之前已经提取到全部的头部，如果还有则只能是包体。
        n = u->buffer.last - u->buffer.pos;
        if (n)
        {
            size_t tmp = n;
            if (u->length == n || tmp >= u->conf->detect_buffer_size) // 最后的报文或者超过检测最小报文需求，直接去检测；
            {
                u->buffer.last = u->buffer.pos;

                u->state->response_length += n;
                // buffer中的剩余缓存的指针放到out_buf中存储。buffer中的pos位置没有法相变化
            ...
            {
                tmp = u->buffer.end - u->buffer.pos;
                if (tmp < u->conf->detect_buffer_size) // 剩余空间不满足要求
                {
                    if (u->buffer.start == u->buffer.pos) // 已经是开始位置，说明缓存空间没有设置正确。直接发送报文
                    {

                        // ngx_log_error(NGX_LOG_ERR, c->log, 0,
                        printf("%s\n",
                               "upstream buffer size is smaller than detect buffer size, please reset!!!");
                    }
                    else
                    {
                        printf("copy from pos, %ld\n", n);
                        ngx_memcpy(u->buffer.start, u->buffer.pos, n);
                        u->buffer.pos = u->buffer.start;
                        u->buffer.last = u->buffer.pos + n;
                    }
                }
            }
        }
...
ngx_http_upstream_process_non_buffered_request(ngx_http_request_t *r, ngx_uint_t do_write)
{
    ...
    for (;;)
    {
        ...
        if (do_write)
        {
            ...
            {
                //将out_bufs中记录的包体发送（实际上分两步，将out_buf挂载到r->out上，然后发送）
                ...
                // 释放out_bufs
                // printf("out_bufs: Ox%p, Ox%p\n", u->out_bufs, u->busy_bufs);
...
//将接收到的响应体也就是buffer中的缓存放到out_bufs中；
//此过程并没有发生复制，只是将out_bufs的尾指针指向了buffer的内存地址；
static ngx_int_t
                       ...
                       ngx_uint_t ft_type)
{
    ...
    if (u->peer.tries == 0 || ((u->conf->next_upstream & ft_type) != ft_type) || (u->request_sent && r->request_body_no_buffering) || (timeout && ngx_current_msec - u->peer.start_time >= timeout))
    {
#if (NGX_HTTP_CACHE)
